...
duration = 1  # saniye
threshHoldValue = 30
#soundNames = ["yasin","ömer","fırtına","elif","göktuğ","hacıali"]
...
def plotAveragesOfSounds():
    ...
    for soundname, i in zip(soundNames, range(len(soundNames))):
        ...
        for sample in range(numOfSounds):
            zeros = getAveragesOfFrames(sample, soundname)  # fonksiyonun tanımı aşağıda.
...
def compareAndPlotDiff():
    ...
    testSoundVector = getAveragesOfFrames()  # test kaydımızın işlemden geçirilmiş mfcc'sini alıyoruz.
